...
* SpringBoot有内置的数据源，druid数据源可以加也可以不加，可以配置成使用Druid数据源
...
- T:泛型，需要修改的数据内容，注意因为是根据ID进行修改，所以**传入的对象中需要有ID属性值**
...
**说明:**这些内容在MP的官方文档中有详细的说明，我们可以查看官方文档类配置
...
增删改查四个操作中，**查询**是非常重要的也是非常复杂的操作，这块需要我们重点学习下，这节我们主要学习的内容有:
...
<img src="assets/1631020283701.png" alt="1631020283701" style="zoom:50%;" />
...
List<Card> cards = userDao.selectList(lambdaQueryWrapper.eq(card!=null, Card::getId, card.getId()));
System.out.println(cards);
```

答案：会报错，很多人都认为程序走到card!=null判断为false时，后面的程序就不执行了，其实当判断为false时后面的card.getId()依旧会执行，此时card为null，card.getId()就是null,getId()，所以当然会报错啦；
...
- pom.xml中添加对应的依赖
  ...
  <project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  	...
  	<groupId>com.isias</groupId>
  	<artifactId>MP_04</artifactId>
  	<version>0.0.1-SNAPSHOT</version>
  	<name>MP_04</name>
  	<description>MP_04</description>
  	<properties>
  		<java.version>1.8</java.version>
  	</properties>
  	<dependencies>
...
- 编写CardDao接口
  ...
  @Mapper
  public interface CardDao extends BaseMapper<Card> {		//注意extends BaseMappser<T>
  }
  ```
...
- 最终创建的项目结构为:
  ...
  <img src="assets\image-20230120122543397.png" alt="image-20230120122543397" style="zoom:50%;" />
...
<img src=" assets\image-20230120122624664.png" alt="image-20230120122624664" style="zoom:50%;" />
...
需要将数据库中id的类型改为bigint

![image-20230120123629376]( assets\image-20230120123629376.png)
...
<img src=" assets\image-20230120123710691.png" alt="image-20230120123710691" style="zoom:50%;" />
...
所有的模型类都可以使用该主键ID策略，只需要在配置文件中添加如下内容:
...
```java
	@Test
	void Deletes(){        //删除指定多条数据
		List<Long> list=new ArrayList<>();
		list.add(1616293629138116609L);
		list.add(1616293912329134082L);

		cardDao.deleteBatchIds(list);
	}
```
...
```java
	...
	void Select(){
		...
		list.add(3L);
		list.add(5L);
		for (Card card : cardDao.selectBatchIds(list)) {
...
<img src=" assets\image-20230120154340474.png" alt="image-20230120154340474" style="zoom:67%;" />
