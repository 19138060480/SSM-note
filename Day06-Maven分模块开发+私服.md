...
![1630771178137](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630771178137.png)

### 步骤2:项目中创建domain包

在`maven_03_pojo`项目中创建`com.itheima.domain`包，并将`maven_02_ssm`中Book类拷贝到该包中

![1630771371487](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630771371487.png)

### 步骤3:删除原项目中的domain包

删除后，`maven_02_ssm`项目中用到`Book`的类中都会有红色提示，如下:

![1630771505703](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630771505703.png)

出错的原因是`maven_02_ssm`中已经将Book类删除，所以该项目找不到Book类，所以报错
...
![1630771987325](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630771987325.png)
...
![1630773180969](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630773180969.png)

安装成功后，在对应的路径下就看到安装好的jar包

![1630773262441](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630773262441.png)
...
![1630773580067](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630773580067.png)

### 步骤2:项目中创建dao包

在`maven_04_dao`项目中创建`com.itheima.dao`包，并将`maven_02_ssm`中BookDao类拷贝到该包中

![1630773695062](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630773695062.png)

在`maven_04_dao`中会有如下几个问题需要解决下:

![1630773958756](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630773958756.png)

* 项目`maven_04_dao`的BookDao接口中Book类找不到报错

  * 解决方案在`maven_04_dao`项目的pom.xml中**添加`maven_03_pojo`项目**
...
* 项目`maven_04_dao`的BookDao接口中，Mybatis的增删改查注解报错
  ...
  * 解决方案在`maven_04_dao`项目的pom.xml中**添加`mybatis`的相关依赖**
...
![1630774696344](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630774696344.png)

再次对`maven_02_ssm`项目进行编译，又会报错，如下:

![1630774780211](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630774780211.png)

和刚才的错误原因是一样的，**maven在仓库中没有找到`maven_04_dao`**,所以此时我们只需要**将`maven_04_dao`安装到Maven的本地仓库**即可。

### 步骤4:将项目安装到本地仓库

将需要被依赖的项目`maven_04_dao`，使用maven的install命令，把其安装到Maven的本地仓库中。

![1630774917743](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630774917743.png)

安装成功后，在对应的路径下就看到了安装好对应的jar包

![1630774946856](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630774946856.png)
...
![1630853726532](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630853726532.png)
...
![1630820964663](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630820964663.png)
...
![1630853443920](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630853443920.png)

如果想更全面的查看Maven中各个坐标的依赖关系，可以点击Maven面板中的`show Dependencies`

![1630853519736](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630853519736.png)
...
![1630854436435](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630854436435.png)
...
![1630854923484](D:/JAVALearning/SSM框架/基础框架8笔记/Maven高级笔记/assets/1630854923484.png)
...
![1630858596147](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630858596147.png)
...
![1630859532119](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630859532119.png)
...
![1630859797123](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630859797123.png)
...
![1630862406709](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630862406709.png)

这样我们就可以解决刚才提到的第一个问题，将子项目中的公共jar包抽取到父工程中进行统一添加依赖，这样做的可以简化配置，并且当父工程中所依赖的jar包版本发生变化，所有子项目中对应的jar包版本也会跟着更新。

![1630943390187](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630943390187.png)

#### 步骤4:优化子项目依赖版本问题

如果把所有用到的jar包都管理在父项目的pom.xml，看上去更简单些，但是这样就会导致有很多项目引入了过多自己不需要的jar包。如上面看到的这张图:

![](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630860344968.png)
...
![1630944335419](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630944335419.png)
...
![1630946592924](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630946592924.png)

##### 步骤2:创建子项目

该项目可以被聚合工程管理，同时会继承父工程。

![1630947082716](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630947082716.png)
...
![image-20230114144010681](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\image-20230114144010681.png)

### 步骤1:父工程中定义属性

```xml
<properties>
    <spring.version>5.2.10.RELEASE</spring.version>		<!--spring.version并不是固定的，随便起，相当于变量-->
</properties>
```
...
<!--定义属性-->
<properties>
    <spring.version>5.2.10.RELEASE</spring.version>		<!--spring相关的版本控制-->
    <junit.version>4.12</junit.version>				<!--junit的属性-->
    <mybatis-spring.version>1.3.0</mybatis-spring.version>		<!--mybatis-spring的属性-->
</properties>
```

## 2.配置文件加载属性(如jdbc.properties)
...
![1630977419627](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630977419627.png)
...
![1630977885030](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630977885030.png)
...
![1630948929828](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630948929828.png)

原因就是Maven发现你的项目为web项目，就会去找web项目的入口web.xml[配置文件配置的方式]，发现没有找到，就会报错。

解决方案1：在maven_02_ssm项目的`src\main\webapp\WEB-INF\`添加一个web.xml文件(简单方案。非标准，但是能用就不要用复杂的)
...
解决方案2: 配置maven打包war时，忽略web.xml检查(较为复杂，是标准的方法，但是需要添加一个插件)
...
![1630981519370](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630981519370.png)

具体如何查看这些属性:

在cmd命令行中输入`mvn help:system`

![1630981585748](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630981585748.png)
...
![image-20230114153618536](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\image-20230114153618536.png)

遇到以上不统一的情况，我们需要配置多环境,maven提供配置多种环境的设定，帮助开发者在使用过程中快速切换环境(SpringBoot会简化)
...
![1630983967960](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630983967960.png)

查看到的结果为:

![](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630977885030.png)
...
![](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630977885031.png)

虽然已经能够实现不同环境的切换，但是每次切换都是需要手动修改，如何来实现在不改变代码的前提下完成环境的切换呢?

#### 步骤5:命令行实现环境切换

![1630984476202](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630984476202.png)

#### 步骤6:执行安装并查看env_test环境是否生效

查看到的结果为`jdbc:mysql://127.3.3.3:3306/ssm_db`

![](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630977885032.png)
...
![1630985300814](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630985300814.png)
...
![1630985411766](D:/JAVALearning/SSM框架/基础框架8笔记/Maven高级笔记/assets/1630985411766.png)
...
![1630986926124](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630986926124.png)
...
![image-20230114171447909](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\image-20230114171447909.png)

使用cmd进入到解压目录下的`nexus-3.30.1-01/bin`,执行如下命令(**注意路径里不要有中文**):

```
nexus.exe /run nexus
```

由于我的路径里有中文，所以我在`nexus-3.30.1-01`文件下执行nexus.exe /run nexus

看到如下内容，说明启动成功。

![1630988939301](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630988939301.png)
...
![1630988857125](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630988857125.png)
...
![1630989320979](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630989320979.png)
...
![1630990244010](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630990244010.png)
...
![1630990538229](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630990538229.png)

上面所说的这些内容，我们需要在本地Maven的配置文件`settings.xml`中进行配置。

#### 步骤1:私服上配置仓库

<img src="D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630991211000.png" alt="1630991211000" style="zoom: 100%;" />
...
![1630991535107](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630991535107.png)
...
![1630992305191](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630992305191.png)
...
![1630992513299](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630992513299.png)

现在发布是在itheima-snapshot仓库中，如果想发布到itheima-release仓库中就需要将项目pom.xml中的version修改成RELEASE即可。

如果想删除已经上传的资源，可以在界面上进行删除操作:

![1630992952378](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630992952378.png)

如果私服中没有对应的jar，会去中央仓库下载，速度很慢。可以配置让私服去阿里云中下载依赖。

![1630993028454](D:\JAVALearning\SSM框架\Spring+SpringMVC笔记\assets\1630993028454.png)
